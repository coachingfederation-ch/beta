import{s as T,S as L,t as R}from"./i18n-CLQBegKv.js";import{c as mt,d as et,e as at,g as ht,h as vt,i as ft,j as yt,k as bt,m as wt,n as st,o as xt,a as Tt,u as Et,p as H,s as It,q as nt,r as Ct}from"./cms-data-TbRm3rQQ.js";const it="coachingfederation.ch";async function kt(){const t=`${window.location.origin}${window.location.pathname}`;await T.auth.signInWithOAuth({provider:"google",options:{redirectTo:t,queryParams:{hd:it}}})}async function Lt(){await T.auth.signOut()}function lt(t){return t?t.toLowerCase().endsWith(`@${it}`):!1}async function rt(){var r,l;const{data:{session:t}}=await T.auth.getSession();if(!t)return null;const a=t.user,e=(a==null?void 0:a.email)||"";if(!lt(e))return await T.auth.signOut(),null;const n=((r=a==null?void 0:a.user_metadata)==null?void 0:r.full_name)||((l=a==null?void 0:a.user_metadata)==null?void 0:l.name)||e.split("@")[0],i=n.split(" ").filter(Boolean).slice(0,2).map(d=>d[0].toUpperCase()).join("");return{email:e,name:n,initials:i||"ED",id:a.id}}function Bt(t){return T.auth.onAuthStateChange((a,e)=>{(async()=>{var r;if(!e){t(null);return}const n=((r=e.user)==null?void 0:r.email)||"";if(!lt(n)){await T.auth.signOut(),t(null);return}const i=await rt();t(i)})()})}const M=["Find a Coach","For Organisations","For Coaches","Insights","Events","About","Member Login","Join","Toggle menu","Search","Building a more human future through professional coaching.","Find a Coach","Coach Directory","Why an ICF Coach?","What is Coaching?","FAQs","Organisations","Why Coaching?","Executive Coaching","Team Coaching","Case Studies","For Coaches","Membership","Credentials","Communities","Mentoring & Supervision","About","Our Vision","Board","Partnerships","Contact","Privacy","Code of Ethics","Imprint","Coaching in action.","Ideas, research and perspectives on the future of coaching, leadership, AI and diversity from ICF Switzerland.","All","Search articles…","No articles found","Try a different search or category filter.","Insights","By","Related articles","Back to Insights","Article not found","The article you are looking for may have been moved or is no longer available.","Something went wrong","We could not load this article. Please try again later.","Back to Insights","Loading articles…","Could not load articles","Please try again later.","Deutsch","Français","Italiano","English"].filter((t,a,e)=>e.indexOf(t)===a),I=["de","fr","it"],$t={de:"Deutsch",fr:"Français",it:"Italiano"},O="coachingfederation.ch";let E=null,w="articles",g=null,C=[],j=[],_=[];const ot=document.getElementById("cms-app");async function Mt(){Bt(a=>{E&&a&&E.id===a.id||(E=a,U())}),E=await rt(),U()}function U(){E?B():_t()}function _t(){ot.innerHTML=`
    <div class="cms-login">
    ...
    </div>`,document.getElementById("googleSignIn").addEventListener("click",()=>{kt()})}function B(){f++,ot.innerHTML=`
    <div class="cms-shell">
      ${At()}
    ...
    <aside class="cms-sidebar">
      ...
      <div class="cms-sidebar-footer">
        <span class="cms-avatar">${E.initials}</span>
        <span style="display:flex;flex-direction:column;min-width:0">
          <span class="cms-editor-name">${p(E.name)}</span>
    ...
    </aside>`}function St(){document.querySelectorAll("[data-view]").forEach(a=>{a.addEventListener("click",()=>{const e=a.getAttribute("data-view");G(e==="editor"&&!g?"articles":e)})});const t=document.getElementById("cmsLogout");t&&t.addEventListener("click",()=>Lt())}function G(t){w=t,B()}function Ht(){const t=document.getElementById("cmsMain");t&&(w==="articles"?Dt(t):w==="editor"?Nt(t):w==="taxonomy"?A(t):w==="site-translations"&&jt(t))}async function Dt(t){const a=f;t.innerHTML='<div style="padding:40px;text-align:center;color:var(--text-muted)">Loading articles…</div>';try{C=await mt()}catch(n){if(a!==f)return;t.innerHTML=`<div class="cms-empty"><h3>Could not load articles</h3><p>${p(n.message)}</p></div>`;return}if(a!==f)return;const e={published:0,draft:0,scheduled:0};for(const n of C)e[n.status]!==void 0&&e[n.status]++;t.innerHTML=`
    ...
    <div class="cms-table-head">
    ...
    </div>`,e=t.map(n=>{var u;const i=`cms-status-${n.status}`,r=n.status.charAt(0).toUpperCase()+n.status.slice(1),l=n.updated_at?Ut(n.updated_at):"",d=((u=n.category)==null?void 0:u.name)||"—";return`
      <div class="cms-table-row" data-id="${n.id}">
        ...
        <span class="cms-article-cat">${p(d)}</span>
      ...
      </div>`}).join("");return a+e}function F(){document.querySelectorAll(".cms-table-row[data-id]").forEach(t=>{t.addEventListener("click",()=>{g=t.getAttribute("data-id"),w="editor",B()})})}async function ct(){try{g=(await bt(E.id)).id,w="editor",B()}catch(t){alert("Could not create article: "+t.message)}}let s={title:"",slug:"",excerpt:"",body:"",author:"",status:"draft",category_id:null,featured_image_url:null,featured_image_alt:"",translations:{}},V=null,dt=!1,S=!1,f=0;async function Nt(t){const a=f;if(!g){t.innerHTML=`
    ...
    <div class="cms-editor-body">
      ...
      ${Ft()}
    </div>`,qt()}function Ft(){const t=j.map(i=>`<option value="${i.id}" ${s.category_id===i.id?"selected":""}>${p(i.name)}</option>`).join(""),a=new Set(s.tagIds||[]),e=_.filter(i=>a.has(i.id)).map(i=>`<span class="cms-tag-chip">${p(i.name)} <button data-tag-remove="${i.id}">×</button></span>`).join(""),n=_.filter(i=>!a.has(i.id)).map(i=>`<span class="cms-tag-cloud-item" data-tag-add="${i.id}">${p(i.name)} +</span>`).join("");return`
    <aside class="cms-settings-panel">
    ...
    </aside>`}function qt(){document.getElementById("backToArticles").addEventListener("click",()=>{g=null,w="articles",B()});const t=document.getElementById("titleInput"),a=document.getElementById("excerptInput"),e=document.getElementById("bodyEditor"),n=document.getElementById("authorInput"),i=document.getElementById("slugInput"),r=document.getElementById("categorySelect"),l=document.getElementById("altInput");t.addEventListener("input",()=>{s.title=t.value,y()}),a.addEventListener("input",()=>{s.excerpt=a.value,y()}),e.addEventListener("input",()=>{s.body=e.innerHTML,y()}),n.addEventListener("input",()=>{s.author=n.value,y()}),i.addEventListener("input",()=>{s.slug=i.value,y()}),r.addEventListener("change",()=>{s.category_id=r.value||null,y()}),l.addEventListener("input",()=>{s.featured_image_alt=l.value,y()}),document.querySelectorAll("[data-cmd]").forEach(o=>{o.addEventListener("click",c=>{c.preventDefault(),document.execCommand(o.getAttribute("data-cmd"),!1,null),e.focus(),s.body=e.innerHTML,y()})}),document.querySelectorAll("[data-block]").forEach(o=>{o.addEventListener("click",c=>{c.preventDefault();const h=o.getAttribute("data-block");h==="h2"||h==="h3"||h==="p"?document.execCommand("formatBlock",!1,h):h==="blockquote"?document.execCommand("formatBlock",!1,"blockquote"):h==="ul"?document.execCommand("insertUnorderedList",!1,null):h==="ol"&&document.execCommand("insertOrderedList",!1,null),e.focus(),s.body=e.innerHTML,y()})});const d=document.getElementById("featuredDrop"),u=document.getElementById("featuredFileInput"),b=document.getElementById("featuredRemove");d.addEventListener("click",o=>{o.target!==b&&u.click()}),d.addEventListener("dragover",o=>{o.preventDefault(),d.classList.add("dragging")}),d.addEventListener("dragleave",()=>d.classList.remove("dragging")),d.addEventListener("drop",o=>{o.preventDefault(),d.classList.remove("dragging");const c=o.dataTransfer.files[0];c&&c.type.startsWith("image/")&&W(c)}),u.addEventListener("change",()=>{const o=u.files[0];o&&W(o)}),b.addEventListener("click",o=>{o.stopPropagation(),s.featured_image_url=null,d.classList.remove("has-image");const c=d.querySelector("img");c&&c.remove(),y()}),document.querySelectorAll("[data-tag-add]").forEach(o=>{o.addEventListener("click",()=>{const c=o.getAttribute("data-tag-add");s.tagIds=[...s.tagIds||[],c],x(document.getElementById("cmsMain"))})}),document.querySelectorAll("[data-tag-remove]").forEach(o=>{o.addEventListener("click",c=>{c.stopPropagation();const h=o.getAttribute("data-tag-remove");s.tagIds=(s.tagIds||[]).filter(pt=>pt!==h),x(document.getElementById("cmsMain"))})});const m=document.getElementById("newTagInput");m&&m.addEventListener("keydown",async o=>{if(o.key==="Enter"){o.preventDefault();const c=m.value.trim();if(!c)return;try{const h=await st(c);_.push(h),s.tagIds=[...s.tagIds||[],h.id],x(document.getElementById("cmsMain"))}catch(h){alert("Could not create tag: "+h.message)}}}),document.getElementById("publishBtn").addEventListener("click",Pt),document.getElementById("deleteBtn").addEventListener("click",Rt);const v=document.getElementById("translateNowBtn");v&&v.addEventListener("click",X),I.forEach(o=>{const c=document.getElementById(`retranslate-${o}`);c&&c.addEventListener("click",()=>X(o))})}async function W(t){const a=document.getElementById("featuredDrop");a.innerHTML='<span style="color:var(--text-muted);font-size:14px">Uploading…</span>';try{const e=await Et(t);s.featured_image_url=e,a.classList.add("has-image"),a.innerHTML=`<img src="${k(e)}" alt=""><button class="cms-featured-remove" id="featuredRemove">Remove image</button><input type="file" id="featuredFileInput" accept="image/*" style="display:none">`,document.getElementById("featuredRemove").addEventListener("click",n=>{var i;n.stopPropagation(),s.featured_image_url=null,a.classList.remove("has-image"),(i=a.querySelector("img"))==null||i.remove(),y()}),y()}catch(e){alert("Upload failed: "+e.message),x(document.getElementById("cmsMain"))}}function y(){dt=!0;const t=document.getElementById("saveStatus");t&&(t.textContent="Saving…",t.className="cms-save-status dirty"),clearTimeout(V),V=setTimeout(D,1500)}async function D(){if(g)try{await H(g,{title:s.title,slug:s.slug,excerpt:s.excerpt,body:s.body,author:s.author,category_id:s.category_id,featured_image_url:s.featured_image_url,featured_image_alt:s.featured_image_alt}),s.tagIds&&await It(g,s.tagIds),dt=!1;const t=document.getElementById("saveStatus");t&&(t.textContent="All changes saved",t.className="cms-save-status saved")}catch(t){const a=document.getElementById("saveStatus");a&&(a.textContent="Save failed: "+t.message,a.className="cms-save-status dirty")}}async function Pt(){var a,e;if(!g)return;if((s.status==="published"?"draft":"published")==="published"){const n=s.title&&s.title.trim()&&s.title.trim()!=="Untitled article",i=s.body&&s.body.replace(/<[^>]*>/g,"").trim().length>0;if(!n||!i){Z("warning","Article has no content","This article is missing a title or body text. It will be published without translations — visitors will see the English version in all languages. Continue?");return}Z("translating");try{await D();const r=`${s.title}|${s.excerpt}|${s.body}`;if(!((e=(a=s.translations)==null?void 0:a.de)!=null&&e.title)||s._translationHash!==r){Q("Translating into German, French and Italian…");const u=[s.title||"",s.excerpt||"",s.body||""];for(const m of I){const[v,o,c]=await R(u,L,m);s.translations[m]={title:v,excerpt:o,body:c}}const b={};for(const m of I){const v=s.translations[m];v&&v.title!=null&&(b[`title_${m}`]=v.title,b[`excerpt_${m}`]=v.excerpt,b[`body_${m}`]=v.body)}await nt(s.id,b,r),s._translationHash=r}else Q("Translations already up to date — publishing…");const d=await H(g,{status:"published",published_at:new Date().toISOString()});s.status="published",$(),x(document.getElementById("cmsMain")),z("Article published with translations.")}catch(r){$(),alert("Could not publish: "+r.message)}}else try{await D(),await H(g,{status:"draft",published_at:null}),s.status="draft",x(document.getElementById("cmsMain"))}catch(n){alert("Could not change publish status: "+n.message)}}function Z(t,a,e){$();const n=document.createElement("div");n.className="cms-publish-overlay",n.id="publishOverlay",t==="translating"?n.innerHTML=`
      ...
      </div>`),document.body.appendChild(n),requestAnimationFrame(()=>n.classList.add("visible")),t==="warning"&&(document.getElementById("publishCancel").addEventListener("click",$),document.getElementById("publishForce").addEventListener("click",async()=>{$(),await Ot()}))}function Q(t){const a=document.getElementById("publishOverlayMessage");a&&(a.textContent=t)}function $(){const t=document.getElementById("publishOverlay");t&&(t.classList.remove("visible"),setTimeout(()=>t.remove(),200))}async function Ot(){try{await D(),await H(g,{status:"published",published_at:new Date().toISOString()}),s.status="published",x(document.getElementById("cmsMain")),z("Article published (no translations generated).")}catch(t){alert("Could not publish: "+t.message)}}async function Rt(){if(g&&confirm("Delete this article? This cannot be undone."))try{await Ct(g),g=null,w="articles",B()}catch(t){alert("Could not delete: "+t.message)}}async function A(t){const a=f;t.innerHTML='<div style="padding:40px;text-align:center;color:var(--text-muted)">Loading…</div>';try{const[e,n,i,r]=await Promise.all([et(),at(),ht(),vt()]);if(a!==f)return;j=e,_=n,t.innerHTML=`
      <div class="cms-page-header">
        <div>
          ...
          <div id="catList">
            ${e.map(l=>`
              <div class="cms-tax-item">
                <span class="cms-tax-item-name">${p(l.name)}</span>
      ...
      </div>`,document.getElementById("addCatBtn").addEventListener("click",J),document.getElementById("newCatInput").addEventListener("keydown",l=>{l.key==="Enter"&&J()}),document.getElementById("addTagBtn").addEventListener("click",K),document.getElementById("newTagInput2").addEventListener("keydown",l=>{l.key==="Enter"&&K()}),document.querySelectorAll("[data-cat-del]").forEach(l=>{l.addEventListener("click",async()=>{const d=l.getAttribute("data-cat-del");try{await ft(d),A(t)}catch(u){alert("Could not delete category: "+u.message)}})}),document.querySelectorAll("[data-tag-del]").forEach(l=>{l.addEventListener("click",async()=>{const d=l.getAttribute("data-tag-del");try{await yt(d),A(t)}catch(u){alert("Could not delete tag: "+u.message)}})})}catch(e){t.innerHTML=`<div class="cms-empty"><h3>Could not load</h3><p>${p(e.message)}</p></div>`}}async function J(){const t=document.getElementById("newCatInput"),a=t.value.trim();if(a)try{await wt(a),t.value="",A(document.getElementById("cmsMain"))}catch(e){alert("Could not add category: "+e.message)}}async function K(){const t=document.getElementById("newTagInput2"),a=t.value.trim();if(a)try{await st(a),t.value="",A(document.getElementById("cmsMain"))}catch(e){alert("Could not add tag: "+e.message)}}async function jt(t){const a=f;t.innerHTML='<div style="padding:40px;text-align:center;color:var(--text-muted)">Loading…</div>';try{const e=await gt();if(a!==f)return;t.innerHTML=`
      <div class="cms-page-header">
        <div>
      ...
      </div>`;const n=document.getElementById("siteTransRunBtn");n&&n.addEventListener("click",zt)}catch(e){t.innerHTML=`<div class="cms-empty"><h3>Could not load</h3><p>${p(e.message)}</p></div>`}}function ut(t){const a={de:"Deutsch",fr:"Français",it:"Italiano"};return I.map(e=>{const n=t[e]||{total:0,translated:0},i=n.total>0?Math.round(n.translated/n.total*100):0,r=i===100;return`
      ...
      </div>`}).join("")}async function gt(){const t={},a=M.length;for(const e of I){const{data:n,error:i}=await T.from("translations").select("source_text").eq("source_lang",L).eq("target_lang",e).in("source_text",M);if(i)throw i;const r=(n||[]).length;t[e]={total:a,translated:r}}return t}async function zt(){const t=f,a=document.getElementById("siteTransRunBtn"),e=document.getElementById("siteTransResult");if(!(!a||!e)){a.disabled=!0,a.textContent="Translating…",e.innerHTML='<p class="cms-site-trans-explain">Translating site text into German, French and Italian…</p>';try{let n=0;for(const r of I){const l=new Set,{data:d}=await T.from("translations").select("source_text").eq("source_lang",L).eq("target_lang",r).in("source_text",M);(d||[]).forEach(c=>l.add(c.source_text));const u=M.filter(c=>!l.has(c));if(u.length===0){e.innerHTML=`<p class="cms-site-trans-explain">${r.toUpperCase()}: already fully translated.</p>`;continue}const b=await R(u,L,r),{data:m}=await T.from("translations").select("source_text").eq("source_lang",L).eq("target_lang",r).in("source_text",u),v=(m||[]).map(c=>c.source_text),o=u.filter(c=>!v.includes(c));n+=v.length,o.length>0&&console.warn(`Site translation: ${o.length} strings for ${r.toUpperCase()} were not persisted`,o)}if(t!==f)return;n===0?e.innerHTML='<div class="cms-site-trans-error">No strings were persisted. Check the browser console for details — the translation service may be unavailable.</div>':e.innerHTML=`<div class="cms-site-trans-success">✓ Translated ${n} strings across all languages. Language switching is now instant on every page.</div>`,a.textContent="Re-run pre-translation",a.disabled=!1;const i=await gt();if(t!==f)return;document.querySelector(".cms-site-trans-card:nth-child(2)").innerHTML='<div class="cms-site-trans-card-header"><h2>Translation status</h2></div>'+ut(i)}catch(n){e.innerHTML=`<div class="cms-site-trans-error">Translation failed: ${p(n.message)}</div>`,a.textContent="Translate site text now",a.disabled=!1}}}function q(t){const a=s.translations[t]||{},e=a.title!=null,n=a.body!=null,i=e&&n;return`<span class="cms-lang-tab ${i?"cms-lang-tab-done":"cms-lang-tab-pending"}">${t.toUpperCase()} ${i?'<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M20 6L9 17l-5-5"></path></svg>':'<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 7v6"></path><circle cx="12" cy="16.5" r="0.5" fill="currentColor"></circle></svg>'}</span>`}function P(t){const a=s.translations[t]||{},e=a.title!=null&&a.body!=null,n=e?"Translated":"Pending",i=e?"cms-tag-status-done":"cms-tag-status-pending";return`<div class="cms-tag-row">
    <span>${$t[t]}</span>
