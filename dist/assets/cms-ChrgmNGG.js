import{s as w}from"./supabase-client-BRH4RlQG.js";import{c as Q,d as N,e as q,g as J,h as K,i as X,j as Y,k as tt,l as et,m as P,n as at,a as st,u as nt,o as F,s as it,p as lt}from"./cms-data-DZNgpiV_.js";const z="coachingfederation.ch";async function dt(){const t=`${window.location.origin}${window.location.pathname}`;await w.auth.signInWithOAuth({provider:"google",options:{redirectTo:t,queryParams:{hd:z}}})}async function ct(){await w.auth.signOut()}function O(t){return t?t.toLowerCase().endsWith(`@${z}`):!1}async function V(){var i,m;const{data:{session:t}}=await w.auth.getSession();if(!t)return null;const e=t.user,a=(e==null?void 0:e.email)||"";if(!O(a))return await w.auth.signOut(),null;const n=((i=e==null?void 0:e.user_metadata)==null?void 0:i.full_name)||((m=e==null?void 0:e.user_metadata)==null?void 0:m.name)||a.split("@")[0],l=n.split(" ").filter(Boolean).slice(0,2).map(c=>c[0].toUpperCase()).join("");return{email:a,name:n,initials:l||"ED",id:e.id}}function rt(t){return w.auth.onAuthStateChange((e,a)=>{(async()=>{var i;if(!a){t(null);return}const n=((i=a.user)==null?void 0:i.email)||"";if(!O(n)){await w.auth.signOut(),t(null);return}const l=await V();t(l)})()})}const M="coachingfederation.ch";let x=null,h="articles",o=null,v=[],A=[],E=[];const R=document.getElementById("cms-app");async function ot(){rt(e=>{x=e,$()}),x=await V(),$()}function $(){x?k():ut()}function ut(){R.innerHTML=`
    <div class="cms-login">
